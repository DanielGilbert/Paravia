...
namespace Paravia
{
    ...
    public static class Game
    {
        ...
        private static List<string> CityList = new()
            {
                "Santa Paravia",
                "Fiumaccio",
                "Torricella",
        ...
        private static List<string> MaleTitles = new()
            {
                "Sir",
                "Baron",
                "Count",
        ...
        private static List<string> FemaleTitles = new()
            {
                "Lady",
                "Baroness",
                "Countess",
        ...
        private static int Random(int hi)
        {
            return Randomizer.Next(hi);
        }

        public static void InitializePlayer(Player player, int year, int city, int level, string name, bool maleOrFemale)
        {
            player.Cathedral = 0;
        ...
        public static async void Run()
        {
            ...
            string? response = await console.ReadLineAsync();
            ...
            response = await console.ReadLineAsync();
            ...
            gameLevel = Convert.ToInt32(await console.ReadLineAsync());
            if (gameLevel < 1)
            {
                gameLevel = 1;
            }
            ...
            for (int i = 0; i < NumberOfPlayers; i++)
            {
                ...
                string? rulersName = await console.ReadLineAsync();
                ...
                string gender = await console.ReadLineAsync();
        ...
        private static async Task PlayGame(List<Player> players, int numberOfPlayers)
        {
            bool AllDead, Winner;
            int WinningPlayer = 0;
            Player Baron;
            ...
            while (AllDead is false && Winner is false)
            {
                for (int i = 0; i < NumberOfPlayers; i++)
                {
                    if (!players[i].IsDead)
                    {
                        await NewTurn(players[i], NumberOfPlayers, players, Baron);
                    }
                }
        ...
        private static async Task NewTurn(Player player, int numberOfPlayers, List<Player> players, Player baron)
        {
            GenerateHarvest(player);
            NewLandAndGrainPrices(player);
            await BuySellGrain(player);
            await ReleaseGrain(player);
            ...
            await AdjustTax(player);
            DrawMap(player);
            await StatePurchases(player, numberOfPlayers, players);
        ...
        private static async void ImDead(Player player)
        {
            int why = 0;
        ...
        private static bool CheckNewTitle(Player player)
        {
            int total = 0;
        ...
        private static void ChangeTitle(Player player)
        {
        ...
        private static int Limit10(int num, int denom)
        {
            int val = num / denom;
            return (val > 10 ? 10 : val);
        }

        private static async Task StatePurchases(Player player, int howMany, List<Player> players)
        {
        ...
        private static void ShowStats(Player player, int howMany, List<Player> players)
        {
        ...
        private static void BuySoldiers(Player player)
        {
            player.Soldiers += 20;
            player.Serfs -= 20;
            player.Treasury -= 500;
        ...
        private static void BuyCathedral(Player player)
        {
            player.Cathedral += 1;
        ...
        private static void BuyPalace(Player player)
        {
            player.Palace += 1;
        ...
        private static void BuyMill(Player player)
        {
            player.Mills += 1;
        ...
        private static void BuyMarket(Player player)
        {
        ...
        private static void DrawMap(Player player)
        {
        ...
        private static async Task AdjustTax(Player player)
        {
            string result = string.Empty;
            int val = 1;
            int duty = 0;
            ...
            while (val != 0 || result[0] != 'q')
            {
                ...
                switch (val)
                {
                    case 1:
                        ...
                        result = await console.ReadLineAsync() ?? "0";
                    ...
                    case 4:
                        ...
                        result = await console.ReadLineAsync() ?? "1";
                        duty = Convert.ToInt32(result);
                        if (duty > 4)
                            duty = 4;
                        if (duty < 1)
        ...
        private static async Task SeizeAssets(Player player)
        {
            string result = string.Empty;
            player.Marketplaces = 0;
            player.Palace = 0;
        ...
        private static void AddRevenue(Player player)
        {
        ...
        private static void GenerateIncome(Player player)
        {
            double y;
            int revenues = 0;
        ...
        private static int AttackNeighbor(Player player, Player opponent)
        {
            int landTaken = 0;
            int deadSoldiers = 0;
        ...
        private static async Task<int> ReleaseGrain(Player player)
        {
            double xp, zp;
            double x, z;
            string result;
        ...
        private static void SerfsProcreating(Player player, double myScale)
        {
            int absc = 0;
            double ord = 0.0;
        ...
        private static void SerfsDecomposing(Player player, double myScale)
        {
            int absc = 0;
            double ord = 0.0;
        ...
        private static async Task BuySellGrain(Player player)
        {
            bool finished = false;
            string result = string.Empty;
            ...
            while (finished is false)
            {
                ...
                result = await console.ReadLineAsync();
                if (result[0] == 'q')
                    finished = true;

                if (result[0] == '1')
                    await BuyGrain(player);

                if (result[0] == '2')
                    await SellGrain(player);

                if (result[0] == '3')
                    await BuyLand(player);

                if (result[0] == '4')
                    await SellLand(player);
            }

        }
        private static async Task BuyGrain(Player player)
        {
            int howMuch;
            string result;
        ...
        private static async Task SellGrain(Player player)
        {
            int howMuch;
            string result;
        ...
        private static async Task BuyLand(Player player)
        {
            int howMuch;
            string result;
        ...
        private static async Task SellLand(Player player)
        {
            int howMuch;
            string result;
        ...
        private static void PrintGrain(Player player)
        {
            console.WriteLine();
        ...
        private static void NewLandAndGrainPrices(Player player)
        {
            double x, y, myRandom;
            int h;
        ...
        private static void GenerateHarvest(Player player)
        {
